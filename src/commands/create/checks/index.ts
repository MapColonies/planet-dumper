...
export const dumpServerUriCheck: CheckFunc<DumpServerConfig> = (argv) => {
...
export const httpHeadersCheck: CheckFunc<DumpServerConfig> = (argv) => {
  const { dumpServerHeaders } = argv;
  if (
    dumpServerHeaders.length > 0 &&
    dumpServerHeaders.some((headerKeyValue) => headerKeyValue.trim().split('=').length !== HEADER_KEY_VALUE_PAIR_LENGTH)
  ) {
